...
class Seq2SeqAttNN(NN):
    ...
    def build_model(self):
        ...
        out_v = tf.reshape(tf.layers.conv2d(inputs, filters=1, kernel_size=[horizontal_length, 1], name="vertical"), [-1, vertical_length])
        out_h = [tf.expand_dims(tf.math.reduce_max(tf.layers.conv2d(inputs, filters=1, kernel_size=[i+1, vertical_length], name=f"horizontal_{i}"), [1, 2, 3], keepdims=False), axis=1) for i in range(horizontal_length)]

        din_out = tf.concat([out_v]+out_h, axis=-1)

        self.w1 = tf.Variable(
            tf.random_normal([vertical_length+horizontal_length, self.edim], stddev=self.stddev),
            trainable=True
        )
