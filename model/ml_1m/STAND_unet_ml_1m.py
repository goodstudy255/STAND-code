...
class Seq2SeqAttNN(NN):
    ...
    def target_attention(self,recent_play, play_history,sub_name="target_attention", debug_name=""):
        with tf.variable_scope("%s" % sub_name, reuse=tf.AUTO_REUSE):
            ...
            din_output = self.simple_dnn(din_input, hidden_units=[4], single_output=True, sub_name="din")
    ...
    def unet(self,keys, length=50 , num_layers=3, out_dims=4, scope="unet"):
        """
        ...
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            for i in range(num_layers-2, -1, -1):
                ...
                x = tf.nn.swish(x)
                x = tf.concat([x, left_blocks[i][:, edge_size:edge_size+tf.shape(x)[1], :]], axis=-1) # 拼接裁剪后的低维信息  -edge_size
                for j in range(num_conv_per_layer):
                    x = tf.layers.conv1d(x, filters=filters[i], kernel_size=3, strides=1, padding='valid', activation=None, kernel_regularizer=tf.keras.regularizers.l1_l2())
                    x = tf.nn.swish(x)
    ...
    def diffusion(self, inputs, input_with_target, is_train, sub_name="diffusion", total_steps=10):
        with tf.variable_scope("%s" % sub_name):
            hidden_dim = inputs.get_shape().as_list()[-1]
            tau, a, b = 1, 2e-2, 1e-2 
            # a = 0.002
            # b = math.log(50)/total_steps
            a = (0.1-0.002)/math.log(total_steps)
            b = math.exp(0.002*math.log(total_steps)/(0.1-0.002))
            alphas, betas, alpha_hats, beta_hats = [], [], [], [] # 控制加噪声的大小，beta越大噪声越大
            for s in range(1, total_steps+1):
                # beta = 0.0002 + (s-1)/total_steps*(1-0.0002)
                # beta = a*s/total_steps + b/s # truncated linear schedule
                # beta = a*math.exp(b*s)
                beta = a*math.log(b*s)
                while beta > tau:
                    beta *= 0.1
            ...
            step_weight = 1e-3
            step_embs = [[math.sin(t/2**f) for f in range(hidden_dim*2)] for t in range(1, total_steps+1)] # 提示模型处于第t步
            ...
            if is_train==1:
                ...
                reconstructed_t = self.unet(noised_t_input + step_weight*step_emb, length = 50, num_layers=4, out_dims=hidden_dim) # 重建为x0 x-prediction
    ...
    def build_train_model(self):
        ...
        self.lab_input_emb = tf.nn.embedding_lookup(self.embe_dict,self.lab_input,max_norm=1.5)
        self.lab_input_tag_emb = tf.matmul(self.lab_input_tag,self.embe_dict_tag)/tf.reduce_sum(self.lab_input_tag,axis=-1,keepdims=True)
        self.lab_input_emb = tf.concat((self.lab_input_emb,self.lab_input_tag_emb),axis=-1)
        ...
        # session_inputs = self.unet(session_inputs,length = 10)
        # session_inputs = tf.math.reduce_mean(session_inputs, 1, True)
        # session_inputs = self.GRU(session_inputs,1,sequence_length = self.sequence_length,batch_size = batch_size)
        # session_inputs = session_inputs[:,-1,:]
        # session_inputs = tf.expand_dims(session_inputs,1)
        ...
        with tf.control_dependencies(print_ops):   
            with tf.name_scope('train_loss'):
                ...
                if KL_loss is not None:
                    ...
                    self.loss = self.loss + self.loss_kl
