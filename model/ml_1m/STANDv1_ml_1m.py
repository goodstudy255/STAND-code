...
class Seq2SeqAttNN(NN):
    ...
    def diffusion(self, inputs, is_train, sub_name="diffusion", total_steps=10):
        with tf.variable_scope("%s" % sub_name):
            ...
            step_weight = 1e-3
            step_embs = [[math.sin(t/2**f) for f in range(hidden_dim)] for t in range(1, total_steps+1)] # 提示模型处于第t步
            ...
            if is_train==1:
                ...
                KL_loss = tf.math.reduce_mean(tf.sqrt(tf.math.reduce_sum(tf.square(inputs-reconstructed_t), axis=-1))) # 实际简化为mse_loss

                predicted_next = reconstructed_t[:, -1:, :] # [B, h]
            else:
                # test graph
                KL_loss = None
                ...
                for t in range(infer_steps-1, -1, -1):           
                    step_emb = tf.convert_to_tensor(step_embs[t], dtype=tf.float32)
                    reconstructed_t = self.unet(noised_t + step_weight*step_emb, num_layers=3, out_dims=hidden_dim) # 重建为x0 x-prediction
                    if t == 0:
    ...
    def build_train_model(self):
        ...
        KL_loss, predicted_next = self.diffusion(extended_input[:, 1: , :], 1)
    ...
    def build_test_model(self):
        ...
        extended_input = tf.concat([inputs, lab_input_emb], axis=1)
        # extended_input = self.simple_dnn(extended_input, sub_name="mlp", hidden_units=[4]) 
        KL_loss, predicted_next = self.diffusion(extended_input[:, 1: , :], 0)
        session_inputs = tf.concat([extended_input[:, -11: -1, :], predicted_next], axis=1)
        history_play_actual_lens = tf.cast(tf.reduce_sum(tf.sign(tf.reduce_max(tf.abs(session_inputs), axis=2, keepdims=True)), axis=1, keepdims=True), tf.float32)
        session_inputs = self.self_attention(session_inputs, history_play_actual_lens, atten_unit=2, head_num=2)
        session_inputs = tf.math.reduce_mean(session_inputs, 1, True)
